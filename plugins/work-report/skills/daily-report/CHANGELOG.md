...
**1. 调整文档结构顺序**
- 将"关键约束"提到最前面（原在第29行）
- 删除独立的"参考文档"表格（改为内联到工作流程）
- 移除"功能模块标识"节（已移至 `../shared/task-type-rules.md`）

**2. 工作流程内联文档触发点**

改进前：只在一处提到"按 detailed-workflow.md 执行"
...
改进前：使用 `/path/to/project` 占位符，Agent 可能困惑

改进后：明确说明"从配置文件的 projects 数组获取项目路径列表"

#### 文档更新

- 重构 `SKILL.md`：85行 → 71行，结构更清晰
- 更新 `../shared/task-type-rules.md`：添加功能模块标识映射表

#### 影响范围

- ✅ Agent 能清晰知道何时读取哪个参考文档
- ✅ 关键约束前置，减少违规操作
- ✅ 文档更精简，减少上下文消耗
